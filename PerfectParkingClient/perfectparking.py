...
class RestApiUtility:
    ...
    @staticmethod
    def update_server_parking_monitor_data(
        parking_monitor_data: ParkingMonitorData,
        free_spaces_in_frame: float,
        probability_parking_available: float,
    ) -> requests.Response:
        """
        ...
        Raises:
            None
        """
        parking_monitor_data_json: dict = (
            RestApiUtility.build_parking_monitor_data_json(
                parking_monitor_data,
                free_spaces_in_frame,
                probability_parking_available,
            )
        )
        return RestApiUtility.build_and_send_parking_monitor_data_put_request(
            parking_monitor_data, parking_monitor_data_json
        )

    @staticmethod
    def build_parking_monitor_data_json(
        parking_monitor_data: ParkingMonitorData,
        free_spaces_in_frame: float,
        probability_parking_available: float,
    ) -> dict:
        ...
        return {
            ...
            "probabilityParkingAvailable": "{:.2f}".format(
                probability_parking_available
            ),
    ...
    @staticmethod
    def build_and_send_parking_monitor_data_put_request(
        parking_monitor_data: ParkingMonitorData, request_json: dict
    ) -> requests.Response:
        ...
        request_url: str = (
            f"{parking_monitor_data.server_url}/{parking_monitor_data.id}/"
        )
        return RestApiUtility.send_put_request(
            parking_monitor_data, request_headers, request_json, request_url
        )
...
def create_image_from_video(image_file_path: str, video_connection_string: str) -> None:
    ...
    display_dimensions: tuple = (
        int(image_dimensions[0] * zoom_factor),
    ...
    while True:
        ...
        display_video_frame = resize(
            video_frame, display_dimensions, interpolation=INTER_CUBIC
        )
